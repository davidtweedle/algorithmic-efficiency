...
def comm_hook(state: LowRankApproximationState, bucket: dist.GradBucket) -> torch.futures.Future[torch.Tensor]:
  if state.getstep() > 1:
    for grad in bucket.gradients():
      ...
      reshaped_grad = grad.reshape(grad_shape[0],-1)
      if (len(reshaped_grad.shape) == 2 and reshaped_grad.shape[0] > 1 
          and reshaped_grad.shape[1] > 1):
        try:
          rank = state.upper_bound_rank if state.upper_bound_rank < reshaped_grad.shape[0] else reshaped_grad.shape[0]
          rank = rank if rank < reshaped_grad.shape[1] else reshaped_grad.shape[1] 
          U,S,V = torch.svd_lowrank(
                  reshaped_grad,
                  q=rank
                  )
          rank = state.svd_rank if state.svd_rank < rank else rank
          U = U[:,:rank]
          S = S[:rank]
          V = V[:,:rank]
